...
export const GOVERNANCE_FILES = ['AGENTS.md','yss-project.yaml','docs/process/analysis-project.yaml','docs/process/implementation-repo-registry.yaml','skills-lock.json','docs/process/acceptance-policy.yaml','docs/process/acceptance-driven-development.md','docs/process/mvc-governance-profile.yaml','docs/process/schemas/lifecycle-checkpoint-v2.schema.json','docs/process/templates/acceptance-checkpoint-template.yaml','scripts/verify-lifecycle-checkpoint','scripts/verify-mvc-governance-profile.mjs','scripts/acceptance-action.mjs','scripts/lib/acceptance-policy.mjs','scripts/lib/lifecycle-transition.mjs','scripts/lib/implementation-contract-compiler.mjs'];
...
export async function recordGovernance(root){
 ...
 await mkdir(path.join(root,'.yss'),{recursive:true});await writeFile(path.join(root,'.yss/governance-files.json'),JSON.stringify({schema_version:1,files},null,2)+'\n');
}
