...
`英文标识` 是 PascalCase 词干，不是英语释义。`## 业务术语` 必须填写该列；`新任务执行默认以 `docs/process/acceptance-policy.yaml` 为准。下表批准、会签与 paused-human-gate 的定义只描述 schema v1 历史兼容；v2 不把技术校验写作用户批准。

## 流程术语` 填 `—`。代码类型名、字段名和契约 property 由该词干变形：类名 = 词干 + YSS 工程后缀；字段与 JSON property 为 camelCase；数据库列为 snake_case；枚举常量为 UPPER_SNAKE。工程后缀以 YSS skill 和当前工程惯例为准，不作为领域词写入本表。JSON 与 Java 字段不一致时，以 DTO wire shape / OpenAPI 为准，不在本表另开列。改中文术语或英文标识都是统一语言变更，必须先回写本表。`避免 / 备注` 同时记录禁用中文和禁用英文别名。
...
| 术语 | 含义 | 英文标识 | 避免 / 备注 |
|---|---|---|---|
| 验收规范 | 项目工程质量基线与需求业务验收条件的组合，供独立审查与实际验证消费。 | — | 工程标准定义一次；Reviewer 不临时增加个人偏好的硬要求。 |
| 自主推进 | 在用户目标授权内连续分析、实现、审查和纠偏，仅实际需求分歧或范围边界需要用户输入。 | — | 不替代外部动作授权；不允许继续使用过期契约。 |
